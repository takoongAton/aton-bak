...
/* 임의 */
const slick_wrap = document.querySelector(".bnr_list .slide_wrap");
if(slick_wrap != null) {
	$('.bnr_list .slide_wrap').slick({
		dots: true,
		dotsClass: "bnr_dots_bottom",
		infinite: true,
		arrows: false
	});
}
...
/* 주소 가져와서 포함텍스트 확인 후 액션 패스로그인 테스트 요청 */
$("a").on("click", function(){
	var getUrl = window.location.href;
	if(getUrl.indexOf("pass_app") > 0) {
		var pass_app = $(".pass_app").offset().top;
		$("html,body").stop().animate({ scrollTop : pass_app }, 300);
	}
})