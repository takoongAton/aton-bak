...
function fn_itemBlind(num){
	...
	$(".itemBlind").css({
		top : itemList.offset().top + (itemHeight * num),
...
/* 구현용 테스트 스크립트 입니다. 개발에 맞게 수정부탁드립니다. */
/* 입력창에 포커스, 또는 입력값이 있을 경우 상위 div에 class="active" 추가 */
/* 입력값이 없이 포커스 아웃 상태일때는 class="active" 삭제 */
/* id이름(test_search)은 수정부탁드립니다. */
$("#test_search").on("focus", function(){
	$(this).closest("div").addClass("active")
})
$("#test_search").on("blur", function(){
	var item = $(this)
	inputTest(item);
})
function inputTest(item) {
	var testTxt = $("#test_search").val();
	
	if(testTxt != "") {
		item.closest("div").addClass("active");
		console.log("내용있음.")
	} else {
		item.closest("div").removeClass("active");
		console.log("내용없음.")
	}
}
inputTest($("#test_search"));